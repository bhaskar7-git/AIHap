...
export const DoctorDashboard: React.FC = () => {
  ...
  // Action: EMERGENCY SWAP (Immediately swap into Room 204 active slot)
  const handleEmergencySwap = async (tokenId: string) => {
    if (window.confirm(`🚨 Execute Emergency Swap?\n\nThis will immediately swap this critical patient into Room 204, and put any currently serving patient on pause at the #1 position in the waiting line.`)) {
      try {
        setActionLoading(true);
        const res = await queueApi.emergencySwap(doctorId, tokenId);
        if (res.data.success) {
          setQueueState(res.data.data.queueState);
          setIsPriorityModalOpen(false);
          setSelectedTokenForPriority(null);
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold border transition-all flex items-center gap-2 ${isAvailable
              ...
              {queueState?.tokens.length === 0 ? (
                ...
                queueState?.tokens.map((token) => {
                  ...
                  return (
                    <tr
                      key={token.id}
                      className={`hover:bg-slate-50/80 transition-colors ${isCurrent ? 'bg-brand-50/50 font-semibold' : ''
                        }`}
                    >
                      <td className="py-3.5 px-4">
                        <span className={`px-2.5 py-1 rounded-lg font-mono font-bold ${isCurrent ? 'bg-brand-600 text-white shadow-sm' : 'bg-slate-100 text-slate-800'
                          }`}>
                          {token.token_number}
                        </span>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {token.status === 'WAITING' && (
                            <>
                              ...
                              <button
                                onClick={() => handleEmergencySwap(token.id)}
                                className="px-2.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-lg text-xs transition-colors shadow-sm flex items-center gap-1"
                                title="Emergency Swap into Room 204 immediately"
                              >
        ...
        <div className="space-y-4">
          <div className="p-4 bg-brand-50/80 border border-brand-200 rounded-2xl space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-full ${selectedTokenForPriority?.priority === 'EMERGENCY'
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <button
              onClick={() => {
                if (selectedTokenForPriority) {
                  handleEmergencySwap(selectedTokenForPriority.id);
                }
              }}
              className="p-4 rounded-2xl border border-rose-400 bg-rose-50 hover:bg-rose-100 text-center space-y-1 transition-all ring-2 ring-rose-400"
            >
              <span className="font-bold text-sm text-rose-800 block">🚨 EMERGENCY SWAP</span>
              <span className="text-[10px] text-rose-700 font-semibold">Instant Room 204 Swap</span>
            </button>
          </div>

          <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-800 space-y-1">
            <strong className="block font-bold">⚡ Emergency Swap Mechanism:</strong>
            <p>
              Clicking <strong>EMERGENCY SWAP</strong> will immediately bring this patient into Room 204. Any patient currently inside will be paused with #1 priority and called immediately after this emergency is treated.
            </p>
          </div>
        </div>
      </Modal>
